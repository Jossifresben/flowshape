...
import { hopf, hopfFibers, JITTER_MAX } from '../../src/patterns/hopf';
import { standardPatternTests, render } from './harness';
import { defaultParams } from '../../src/patterns/registry';

// SPIKE: hopf is not registered in src/patterns/index.ts; importing the
// module registers it in the module-local registry for these tests only.
standardPatternTests(hopf, { maxElements: 48 });
...
describe('hopf specifics', () => {
  it('phase 1 is byte-identical to phase 0 (R(1) = −I fixes every fiber; % 1 folds the bytes)', () => {
    const p = defaultParams(hopf);
    expect(render(hopf, { ...p, phase: 1 }, 3)).toBe(render(hopf, { ...p, phase: 0 }, 3));
  ...
  it('phase 0 is the resting composition and mid-cycle differs from it', () => {
    const p = defaultParams(hopf);
    expect(render(hopf, { ...p, phase: 0.5 }, 1)).not.toBe(render(hopf, p, 1));
  });
  ...
  it('is non-empty and non-exploded at every single-param extreme, across phases', () => {
    const combos: Record<string, number>[] = [defaultParams(hopf)];
    for (const pd of hopf.params) {
      combos.push({ ...defaultParams(hopf), [pd.key]: pd.min });
      combos.push({ ...defaultParams(hopf), [pd.key]: pd.max });
    }
    for (const c of combos) {
      for (const phase of [0, 0.31, 0.5, 0.77]) {
        const pts = pathPoints(render(hopf, { ...c, phase }, 5));
  ...
  it('above the threshold fibers cross the pole and the clip bounds every coordinate', () => {
    const p = { ...defaultParams(hopf), tilt: 1, pole: 1, spread: 1 };
    let nulls = 0;
    for (let k = 0; k < 40; k++) {
    ...
    const svg = render(hopf, { ...p, phase: 0.4 }, 1);
    expect(svg).not.toContain('NaN');
    expect(svg).not.toContain('Infinity');
  });

  it('pole > 1 pulls the outer rings in', () => {
    const p = { ...defaultParams(hopf), tilt: 1, spread: 1, phase: 0.5 };
